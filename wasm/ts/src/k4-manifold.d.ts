...
export class K4Engine {
  ...
  step_submission(doc0: string, corpus_json: string): any; // Validator single-submission entry
  free(): void;                        // wasm-bindgen adds this on every class
}

// Factory exported by the wasm module — mirrors the Rust-side helper the
// Airlock uses to hydrate the engine from a persisted VFS snapshot.
